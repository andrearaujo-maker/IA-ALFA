...
import requests, threading, time, json, os, math
...
FETCH_INTERVAL = 5
LOOKBACK_MAX = 300
STATE_FILE = "state_ml_state.json"
...
def save_state():
    try:
        ...
        obj = {"pattern_db": pdb_plain, "signals": signals[-MAX_SIGNALS_KEEP:], "stats": stats,
...
def update_history():
    ...
    if len(numeric_history) > LOOKBACK_MAX:
        numeric_history = numeric_history[-LOOKBACK_MAX:]; gp_history = gp_history[-LOOKBACK_MAX:]
    return changed
...
def predict_next():
    p_pred, p_conf = predict_from_history(gp_history[-LOOKBACK_MAX:])
