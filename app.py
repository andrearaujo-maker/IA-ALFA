# app.py
# Painel IA WinGo - Aprendizado online leve + painel Flask
# Atualiza a cada 5s, busca API POPBRA, exibe previsão e histórico.
...
FETCH_INTERVAL = 5           # atualizar a cada 5 segundos
...
def predict_from_history(gp_list):
    ...
    if g > p: return 'P', int(round((g/(g+p))*40))
    elif p > g: return 'G', int(round((p/(g+p))*40))
...
def evaluate_pending():
    ...
    actual = gp_history[-1]
    last_sig["actual"] = actual; last_sig["evaluated"] = True
...
def predicao():
    ...
    return jsonify({"prediction": pred, "confidence": conf, "next_issue": next_issue,
                    "last_results": last_results, "signals": recent_signals, "stats": stats})
